...
import { 
  ...
  Send, Save, Trash2, RefreshCw, X, Feather, Loader2
} from 'lucide-react';
...
// Parses text containing <b> tags and renders them with a fluorescent style
    ...
    <span>
      {parts.map((part, i) => {
        if (part.startsWith('<b>') && part.endsWith('</b>')) {
          return (
            <span key={i} className="bg-yellow-300 text-dark px-1 rounded-sm mx-0.5 font-bold shadow-sm box-decoration-clone">
              {part.replace(/<\/?b>/g, '')}
            </span>
          );
        }
    ...
    >
      <div className={`relative w-full h-full duration-500 transform-style-3d transition-transform ${isFlipped ? 'rotate-y-180' : ''}`}>
        ...
        <div className="absolute w-full h-full backface-hidden bg-white rounded-3xl shadow-xl border-4 border-secondary flex flex-col items-center justify-center p-6 text-center">
          {entry.imageUrl ? (
            <img src={entry.imageUrl} alt={entry.term} className="w-32 h-32 object-cover rounded-full mb-6 border-4 border-yellow-100 bg-gray-50" />
          ) : (
            <div className="w-32 h-32 rounded-full mb-6 border-4 border-yellow-100 bg-gray-50 flex items-center justify-center">
...
const App = () => {
  ...
  // Learn Mode State
  const [learnCardId, setLearnCardId] = useState<string | null>(null);
  ...
  // Story State (Collection)
  const [story, setStory] = useState<string | null>(null);
  const [isStoryLoading, setIsStoryLoading] = useState(false);

  // Story Modal State (Single Word)
  ...
  const handleGenerateSingleStory = async (entry: DictionaryEntry, e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    if (!nativeLang || !targetLang) return;

    setStoryModal({ isOpen: true, content: '', isLoading: true, word: entry.term });
    
    try {
      const result = await generateStoryFromWords([entry], nativeLang, targetLang);
      setStoryModal(prev => ({ ...prev, content: result, isLoading: false }));
    } catch (error) {
      setStoryModal(prev => ({ ...prev, content: "Sorry, I couldn't write a story right now.", isLoading: false }));
    }
  };
  ...
  const handleNextCard = () => {
    ...
    setLearnCardId(randomWord.id);
  };

  const handleNotebookItemClick = (wordId: string) => {
    setLearnCardId(wordId);
    setView('learn');
  };
  ...
  // Determine active flashcard
  const activeFlashcardEntry = learnCardId 
    ? savedWords.find(w => w.id === learnCardId) 
    : savedWords.length > 0 ? savedWords[0] : null;
    ...
    <div className="flex flex-col h-screen bg-gray-50 max-w-md mx-auto shadow-2xl overflow-hidden relative">
      ...
      <main className="flex-1 overflow-y-auto overflow-x-hidden no-scrollbar pb-24 relative">
              ...
              <div className="space-y-6 animate-fade-in pb-20">
                ...
                <div className="bg-white rounded-3xl p-6 shadow-md border-b-4 border-gray-100 relative">
                  ...
                  {currentResult.imageUrl ? (
                    ...
                    <div className="mb-6 rounded-2xl overflow-hidden border-2 border-gray-50 shadow-inner bg-gray-100 h-48 flex items-center justify-center">
                  ...
                  <h2 className="text-3xl font-display font-bold text-dark mb-1 flex items-center gap-2">
                    ...
                    <button 
                        onClick={(e) => handleGenerateSingleStory(currentResult, e)}
                        className="text-purple-400 hover:text-purple-600 transition-colors p-1 rounded-full hover:bg-purple-50"
                        title="Create Story"
                    >
                        <Feather size={20} />
                    </button>
              ...
              <>
                ...
                <div className="grid grid-cols-1 gap-3">
                  {savedWords.map(word => (
                    <div 
                      key={word.id} 
                      onClick={() => handleNotebookItemClick(word.id)}
                      className="bg-white p-3 rounded-2xl shadow-sm flex justify-between items-center cursor-pointer hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-100 group"
                    >
                      <div className="flex items-center gap-3 overflow-hidden">
                        {word.imageUrl ? (
                          <img src={word.imageUrl} alt={word.term} className="w-14 h-14 rounded-xl object-cover bg-gray-100 shrink-0 border border-gray-100" />
                        ) : (
                          <div className="w-14 h-14 rounded-xl bg-gray-100 flex items-center justify-center text-xl shrink-0 border border-gray-100">
                             <Sparkles className="text-gray-300" size={20} />
                          </div>
                        )}
                        <div className="min-w-0">
                          <p className="font-bold text-lg text-dark truncate">{word.term}</p>
                          <p className="text-xs text-gray-400 truncate">{word.explanation}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-1 items-center shrink-0 ml-2">
                         ...
                         <button 
                            onClick={(e) => handleGenerateSingleStory(word, e)}
                            className="text-purple-400 hover:text-purple-600 p-1 rounded-full hover:bg-purple-50 transition-colors"
                            title="Create Story"
                         >
                            <Feather size={18} />
                         </button>
        ...
        {view === 'learn' && (
          <div className="p-4 h-full flex flex-col justify-center">
            {savedWords.length > 0 && activeFlashcardEntry ? (
              <div className="space-y-8 animate-fade-in">
                 ...
                 <Flashcard entry={activeFlashcardEntry} />
                 
                 <div className="flex justify-center">
                   <button 
                     onClick={handleNextCard}
                     className="flex items-center gap-2 bg-dark text-white px-6 py-3 rounded-full shadow-lg hover:bg-gray-800 transition-all active:scale-95"
                   >
      ...
      {storyModal.isOpen && (
        <div className="absolute inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl w-full max-w-sm p-6 relative shadow-2xl flex flex-col max-h-[80vh]">
            <button 
              onClick={() => setStoryModal(prev => ({...prev, isOpen: false}))}
              className="absolute top-4 right-4 text-gray-400 hover:text-dark p-1 bg-gray-100 rounded-full"
            >
              <X size={20} />
            </button>
            
            <h3 className="text-xl font-display font-bold text-purple-600 mb-2 flex items-center gap-2">
              <Feather className="w-5 h-5" />
              Story Time
            </h3>
            <p className="text-xs text-gray-400 mb-4 uppercase tracking-wider font-bold">Word: {storyModal.word}</p>
            
            <div className="flex-1 overflow-y-auto no-scrollbar">
                {storyModal.isLoading ? (
                <div className="flex flex-col items-center justify-center py-10 text-gray-400 gap-3">
                    <Loader2 size={32} className="animate-spin text-purple-400" />
                    <p className="text-sm font-medium">Weaving a magical tale...</p>
                </div>
                ) : (
                <div className="bg-purple-50 p-4 rounded-2xl text-dark text-md leading-relaxed whitespace-pre-wrap border border-purple-100">
                    <HighlightedText text={storyModal.content} />
                </div>
                )}
            </div>
          </div>
      ...
      <nav className="bg-white border-t border-gray-100 flex justify-around items-center p-4 z-10 pb-6">
        ...
        <button 
          onClick={() => {
            setView('learn');
            // If we have words, pick a random one to start "fresh"
            if (savedWords.length > 0) {
              const random = savedWords[Math.floor(Math.random() * savedWords.length)];
              setLearnCardId(random.id);
            }
          }}
